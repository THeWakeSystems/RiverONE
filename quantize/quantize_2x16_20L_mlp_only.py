...
"""
=============================================================================
 RiverOne-QC-4B-v2 AQLM 量化 — 2×16 scheme, 后20层, MLP only
...
qm.OUTPUT_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "RiverOne-QC-4B-v2-AQLM-2x16-20L-MLPonly")
qm.LOG_FILE = os.path.join(SCRIPT_DIR, "quantize_2x16_20L_mlp_only.log")

qm.NUM_LAST_LAYERS = 20  # L16-L35
qm.NUM_CODEBOOKS = 2
...
def _patched_save(model, output_dir, source_dir):
    ...
    if os.path.exists(qc_path):
        with open(qc_path, "r") as f:
            qc = json.load(f)
        qc["scheme"] = "2x16-MLPonly-20L"
        qc["num_codebooks"] = 2
        qc["nbits_per_codebook"] = 16
        qc["in_group_size"] = 16
        qc["out_group_size"] = 1
        qc["quantized_layers"] = "last_20_of_36_llm_mlp_only"
        qc["quantized_components"] = "mlp_gate_up_down"
        qc["preserved_components"] = "attention_qkv_o, layers_0..15, miniViT, embedding, lm_head, norms"
...
if __name__ == "__main__":
    ...
    qm.logger.info("=" * 60)
    qm.logger.info(" AQLM 2×16 量化 — 后20层 MLP only (FAISS k-means++)")
    ...
    qm.logger.info(f" 量化层数: 3 (gate/up/down) × 20 = 60 层")
