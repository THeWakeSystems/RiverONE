...
"""
=============================================================================
 RiverOne-QC-4B-v2 AQLM 量化 — 2×16 scheme, 后28层, MLP only
...
qm.OUTPUT_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "RiverOne-QC-4B-v2-AQLM-2x16-28L-MLPonly")
qm.LOG_FILE = os.path.join(SCRIPT_DIR, "quantize_2x16_28L_mlp_only.log")

qm.NUM_LAST_LAYERS = 28  # L8-L35
qm.NUM_CODEBOOKS = 2
...
def _patched_save(model, output_dir, source_dir):
    ...
    if os.path.exists(qc_path):
        ...
        qc["in_group_size"] = 16
        qc["out_group_size"] = 1
        qc["quantized_layers"] = "last_28_of_36_llm_mlp_only"
        qc["quantized_components"] = "mlp_gate_up_down"
        qc["preserved_components"] = "attention_qkv_o, layers_0..7, miniViT, embedding, lm_head, norms"
...
if __name__ == "__main__":
    ...
    qm.logger.info("=" * 60)
    qm.logger.info(" AQLM 2×16 量化 — 后28层 MLP only (FAISS k-means++)")
    ...
    qm.logger.info(f" 量化层数: 3 (gate/up/down) × 28 = 84 层")
