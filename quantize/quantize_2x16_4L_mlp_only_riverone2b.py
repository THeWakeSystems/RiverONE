...
"""
=============================================================================
 RiverONE-2B-ICL AQLM 量化 — 2×16 scheme, 后4层, MLP only
 ...
 方案定义:
   ...
   量化范围: L24-L27 (最后4层，总28层)

 模型架构: RiverONE-2B-ICL = Qwen3-1.7B (28层, hidden=2048, intermediate=6144)
...
# quantize.py 的 modelutils 只支持 internvl_chat/llama 等类型，
# 需要对 get_llm_model/get_llm_config/get_layers 打补丁
# ═══════════════════════════════════════════════════════════════
...
def _patched_get_llm_config(model):
    if getattr(model.config, "model_type", None) == "riverone_qc":
        # llm_config 已经是 Qwen3Config 对象，直接返回
...
qm.SOURCE_MODEL_PATH = "/home/lxy/workspace/RiverONE-2B-ICL"
qm.OUTPUT_DIR = os.path.join(
    os.path.dirname(SCRIPT_DIR), "RiverONE-2B-ICL-AQLM-2x16-4L-MLPonly"
)
qm.LOG_FILE = os.path.join(SCRIPT_DIR, "quantize_2x16_4L_riverone2b.log")

qm.NUM_LAST_LAYERS = 4
qm.NUM_CODEBOOKS = 2
qm.NBITS_PER_CODEBOOK = 16
qm.NSAMPLES = 64
qm.MODEL_SEQLEN = 2048
qm.OFFLOAD_ACTIVATIONS = False
# ★ FAISS K-Means (k-means++ init + GPU 加速)
qm.USE_FAISS = True
...
# LLM_KEYWORDS 已经包含 "language_model"，不需要修改
# 但确保 riverone_qc 的 language_model 能被找到
...
# ═══════════════════════════════════════════════════════════════
# 修正 quant_config.json 和 files_to_copy
# ═══════════════════════════════════════════════════════════════
...
def _patched_save(model, output_dir, source_dir):
    ...
    if os.path.exists(qc_path):
        with open(qc_path, "r") as f:
            qc = json.load(f)
        qc["scheme"] = "2x16-MLPonly-4L"
        qc["num_codebooks"] = 2
        qc["nbits_per_codebook"] = 16
        qc["in_group_size"] = 16
        qc["out_group_size"] = 1
        qc["quantized_layers"] = "last_4_of_28_llm_mlp_only"
        qc["quantized_components"] = "mlp_gate_up_down"
        qc["preserved_components"] = (
            "attention_qkv_o, layers_0..23, IsingViT, embedding, lm_head, norms"
        )
        qc["source_model"] = "RiverONE-2B-ICL (Qwen3-1.7B + IsingViT)"
        ...
        qm.logger.info("[保存] quant_config.json 已更新 (2x16-MLPonly-4L RiverONE-2B)")
...
if __name__ == "__main__":
    ...
    qm.logger.info("=" * 60)
    qm.logger.info(" AQLM 2×16 量化 — RiverONE-2B-ICL, 后4层 MLP only")
    qm.logger.info(f" scheme: 2×16 (2 codebooks, nbits=16, in_group=16)")
    qm.logger.info(f" codebook_size=65536, 等效位宽 ~3.4 bit/param (MLP)")
    qm.logger.info(f" ★ FAISS K-Means: k-means++ init, 100 iter, max_points/centroid=5")
    qm.logger.info(f" ★ Attention (q/k/v/o_proj) 不量化 — 保持 bf16")
    qm.logger.info(f" 量化层数: 3 (gate/up/down) × 4 = 12 层")
    qm.logger.info(f" 量化范围: L24-L27 (最后4层，总28层)")
