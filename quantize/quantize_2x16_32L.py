...
"""
=============================================================================
 RiverOne-QC-4B-v2 AQLM 量化 — 2×16 scheme, 后32层, MLP only
 ...
 关键改动:
   - Attention 层不量化，避免 nbits=8 codebook 导致的 81-86% 权重误差
   - MLP 层用 nbits=16 (codebook_size=65536)，权重误差 39-48%
   - 校准时加载图像，经过 IsingViT 视觉编码器 + mlp1 投影
   - 捕获 LLM 第一层的多模态 hidden states（文本+视觉拼接）
   - IsingViT 保持原始精度，不压缩
   - K-Means 使用全量校准数据 (max_points_per_centroid=None)
=============================================================================
"""
...
qm.OUTPUT_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "RiverOne-QC-4B-v2-AQLM-2x16-8L-MLPonly")
qm.LOG_FILE = os.path.join(SCRIPT_DIR, "quantize_2x16_8L_mlp_only.log")
...
if __name__ == "__main__":
    ...
    qm.logger.info("=" * 60)
    qm.logger.info(" AQLM 2×16 量化 — 后8层 MLP only (FAISS k-means++)")
