#!/usr/bin/env python3
"""ViT Swap: AQLM MLP-only LLM + miniViT_distilled ViT.

Source models:
  - LLM donor: AQLM/RiverOne-QC-4B-v2-AQLM-2x16-32L-MLPonly (quantized MLP, full bf16 attn)
  - ViT donor: /home/lxy/miniViT_distilled/ (miniViT with weight sharing + distilled transforms)

Output: AQLM-quantized MLP-only LLM + miniViT'd ViT.
...
# --- Config ---
LLM_DONOR = Path("/home/lxy/workspace/RiverOne-QC-4B-v1-AQLM-miniViT/AQLM/RiverOne-QC-4B-v2-AQLM-2x16-32L-MLPonly")
VIT_DONOR = Path("/home/lxy/miniViT_distilled")
OUTPUT_DIR = Path("/home/lxy/workspace/RiverOne-QC-4B-v1-AQLM-miniViT/AQLM/RiverOne-QC-4B-v2-AQLM-2x16-32L-MLPonly_miniViT")
...
print(f"  LLM donor (AQLM MLPonly): {len(llm_tensors)} keys total")
...
# --- Verification ---
print(f"\n=== Verification ===")
print(f"  LLM keys (from AQLM MLPonly):     {len(llm_donor_llm)}  (AQLM: {aqlm_count}, bf16: {bf16_count})")
...
# Verify AQLM tensor integrity
codes_dtypes = set()
for k, v in merged.items():
    if k.endswith('.codes'):
        codes_dtypes.add(str(v.dtype))
    if k.endswith('.codebooks'):
        codes_dtypes.add(str(v.dtype))
print(f"  AQLM codes/codebooks dtypes:      {codes_dtypes}")
