...
"""
=============================================================================
 RiverOne-QC-4B-v2 AQLM 量化 — 2×16 scheme, 后32层, Attn(n14)+MLP(n16)
...
qm.OUTPUT_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "RiverOne-QC-4B-v2-AQLM-2x16-32L-Attn14MLP16")
qm.LOG_FILE = os.path.join(SCRIPT_DIR, "quantize_2x16_32L_attn14_mlp16.log")

qm.NUM_LAST_LAYERS = 32  # L4-L35
qm.NUM_CODEBOOKS = 2
...
def _patched_save(model, output_dir, source_dir):
    ...
    if os.path.exists(qc_path):
        with open(qc_path, "r") as f:
            qc = json.load(f)
        qc["scheme"] = "2x16-Attn14MLP16-32L"
        qc["num_codebooks"] = 2
        qc["nbits_per_codebook"] = 16
        qc["in_group_size"] = 16
        qc["out_group_size"] = 1
        qc["quantized_layers"] = "last_32_of_36_llm_attn14_mlp16"
        qc["quantized_components"] = "attn_qkv_o + mlp_gate_up_down"
        qc["preserved_components"] = "attention_qkv_o, layers_0..3, miniViT, embedding, lm_head, norms"
...
if __name__ == "__main__":
    ...
    qm.logger.info("=" * 60)
    qm.logger.info(" AQLM 2×16 量化 — 后32层 Attn14+MLP16 (FAISS k-means++)")
    ...
    qm.logger.info(f" 量化层数: 3 (gate/up/down) x 32 = 224 层")
