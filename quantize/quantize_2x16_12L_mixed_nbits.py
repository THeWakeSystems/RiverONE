...
"""
=============================================================================
 RiverOne-QC-4B-v2 AQLM 量化 — 2x16 scheme, 后12层, MLP only, mixed nbits
 校准数据: qcaleval_zs_sft.jsonl (多模态：图像→IsingViT→LLM hidden states)

 方案定义:
   num_codebooks=2, in_group_size=16, out_group_size=1
   ★ 混合 nbits: L24-27 用 nbits=14 (codebook=16384), L28-35 用 nbits=16
   ★ 仅量化 MLP (gate_proj, up_proj, down_proj) — Attention 保持 bf16
   量化范围: L24-L35 (最后12层), 共 36 个 AQLM 模块
=============================================================================
"""
...
qm.OUTPUT_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "RiverOne-QC-4B-v2-AQLM-2x16-12L-MLPonly-mixed")
qm.LOG_FILE = os.path.join(SCRIPT_DIR, "quantize_2x16_12L_mixed_nbits.log")

qm.NUM_LAST_LAYERS = 12  # L24-L35 (L24-27:n14, L28-35:n16)
qm.NUM_CODEBOOKS = 2
...
def _patched_save(model, output_dir, source_dir):
    ...
    if os.path.exists(qc_path):
        with open(qc_path, "r") as f:
            qc = json.load(f)
        qc["scheme"] = "2x16-MLPonly-12L-mixed"
        qc["num_codebooks"] = 2
        qc["nbits_per_codebook"] = 16
        qc["in_group_size"] = 16
        qc["out_group_size"] = 1
        qc["quantized_layers"] = "last_12_of_36_llm_mlp_only_mixed_nbits"
        qc["quantized_components"] = "mlp_gate_up_down"
        qc["preserved_components"] = "attention_qkv_o, layers_0..23, miniViT, embedding, lm_head, norms"
        qc["source_model"] = "RiverOne-QC-4B-v2 (miniViT)"
        qc["kmeans"] = "FAISS k-means++, 100 iter, max_points_per_centroid=5, mixed_nbits"
...
# Layer nbits mapping
LAYER_NBITS_MAP = {}
for l in range(36 - 12, 36 - 8):   # L24-L27
    LAYER_NBITS_MAP[l] = 14
for l in range(36 - 8, 36):         # L28-L35
    LAYER_NBITS_MAP[l] = 16

_orig_qsl_inner = qm.quantize_single_layer
def _nbits_aware_qsl(layer, layer_idx, inps, outs, args, forward_args, model):
    nbits = LAYER_NBITS_MAP.get(layer_idx, args.nbits_per_codebook)
    old_nbits = args.nbits_per_codebook
    args.nbits_per_codebook = nbits
    if old_nbits != nbits:
        qm.logger.info(f"  [nbits] layer {layer_idx}: nbits={nbits}")
    try:
        return _orig_qsl_inner(layer, layer_idx, inps, outs, args, forward_args, model)
    finally:
        args.nbits_per_codebook = old_nbits

qm.quantize_single_layer = _nbits_aware_qsl

if __name__ == "__main__":
    os.environ.setdefault("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")

    qm.logger.info("=" * 60)
    qm.logger.info(" AQLM 2x16 量化 — 后12层 MLP only (L24-27:n14, L28-35:n16)")
    ...
    qm.logger.info(f" 量化层数: 3 (gate/up/down) x 12 = 36 层 (mixed nbits)")
