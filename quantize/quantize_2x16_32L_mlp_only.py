...
qm.OUTPUT_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "RiverOne-QC-4B-v2-AQLM-2x16-32L-MLPonly")
qm.LOG_FILE = os.path.join(SCRIPT_DIR, "quantize_2x16_32L_mlp_only.log")
...
def _patched_save(model, output_dir, source_dir):
    ...
    if os.path.exists(qc_path):
        ...
        qc["in_group_size"] = 16
        qc["out_group_size"] = 1
        qc["quantized_layers"] = "last_32_of_36_llm_mlp_only"
...
if __name__ == "__main__":
    ...
    qm.logger.info("=" * 60)
    qm.logger.info(" AQLM 2×16 量化 — 后32层 MLP only (FAISS k-means++)")
    ...
    qm.logger.info(f" 量化层数: 3 (gate/up/down) × 32 = 96 层")
