...
=============================================================================
 ...
 ★ FAISS K-Means nredo=5 优化版 ★
 校准数据: qcaleval_zs_sft.jsonl (多模态：图像→IsingViT→LLM hidden states)

 与基准配置的区别:
   - FAISS nredo=5: 运行5次 K-Means 选最优，降低局部最优风险
   - K-Means 耗时约为基准的 5 倍，但聚类质量更优
=============================================================================
"""
...
qm.OUTPUT_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "RiverOne-QC-4B-v2-AQLM-2x16-8L-MLPonly-nredo5")
qm.LOG_FILE = os.path.join(SCRIPT_DIR, "quantize_2x16_8L_nredo5.log")
...
qm.FAISS_NREDO = 5  # ★ 运行5次K-Means选最优
...
def _patched_save(model, output_dir, source_dir):
    ...
    if os.path.exists(qc_path):
        with open(qc_path, "r") as f:
            qc = json.load(f)
        qc["scheme"] = "2x16-MLPonly-8L-nredo5"
        qc["num_codebooks"] = 2
        qc["nbits_per_codebook"] = 16
        qc["in_group_size"] = 16
        qc["out_group_size"] = 1
        qc["quantized_layers"] = "last_8_of_36_llm_mlp_only"
        ...
        qc["kmeans"] = "FAISS k-means++, nredo=5, 100 iter, max_points_per_centroid=5"
...
if __name__ == "__main__":
    ...
    qm.logger.info("=" * 60)
    qm.logger.info(" AQLM 2×16 量化 — 后8层 MLP only (FAISS nredo=5)")
    qm.logger.info(f" scheme: 2×16 (2 codebooks, nbits=16, in_group=16)")
    qm.logger.info(f" codebook_size=65536, 等效位宽 ~3.4 bit/param (MLP)")
    qm.logger.info(f" ★ FAISS K-Means: nREDO={qm.FAISS_NREDO}, k-means++ init, {qm.INIT_MAX_ITER} iter, max_points/centroid={qm.INIT_MAX_POINTS_PER_CENTROID}")
