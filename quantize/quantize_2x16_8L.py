...
"""
=============================================================================
 RiverOne-QC-4B-v2 AQLM 量化 — 2×16 scheme, 后8层
 ...
 方案定义:
   ...
   等效位宽: 2×16 / (1×16) = 2 bit/param
   codebook 形状: [2, 65536, 1, 16] — 两个 16-bit 码本, 各 65536 个候选向量
   attention 层 nbits=8 — 降低小层 codebook 开销

 关键改动 (vs 2x16_32L):
   - NUM_LAST_LAYERS = 8 (仅量化后8层, 层号 28~35)
   - 源模型: /home/qxyg/RiverOne-QC-4B-v2/
   - 校准数据: /home/qxyg/datasets/vqa_format/qcaleval_zs_sft.jsonl
=============================================================================
"""
...
qm.SOURCE_MODEL_PATH = "/home/qxyg/RiverOne-QC-4B-v2"
qm.OUTPUT_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "RiverOne-QC-4B-v2-AQLM-2x16-8L-MLP")
qm.LOG_FILE = os.path.join(SCRIPT_DIR, "quantize_2x16_8L_mlp.log")

qm.NUM_LAST_LAYERS = 8                     # ★ 仅量化最后8层 (36层中的层28-35)
qm.NUM_CODEBOOKS = 2
qm.NBITS_PER_CODEBOOK = 16
qm.ATTENTION_NBITS_PER_CODEBOOK = None     # ★ 不量化 attention，无需此参数
qm.NSAMPLES = 64
qm.MODEL_SEQLEN = 2048
qm.OFFLOAD_ACTIVATIONS = False
qm.INIT_MAX_POINTS_PER_CENTROID = None     # 全量校准数据参与 K-Means

# ★ 仅量化 MLP 层，跳过 attention (q_proj, k_proj, v_proj, o_proj)
qm.LINEAR_LAYER_KEYWORDS = ["gate_proj", "up_proj", "down_proj"]

CALIBRATION_JSONL = "/home/qxyg/datasets/vqa_format/qcaleval_zs_sft.jsonl"
IMAGE_BASE = "/home/qxyg/datasets/vqa_format"
...
def _patched_save(model, output_dir, source_dir):
    ...
    if os.path.exists(qc_path):
        ...
        qc["quantized_components"] = "mlp_only_gate_up_down"
        qc["in_group_size"] = 16
        qc["out_group_size"] = 1
        qc["quantized_layers"] = "last_8_of_36_llm"
        qc["source_model"] = "RiverOne-QC-4B-v2 (miniViT)"
        qc["calibration_data"] = "qcaleval_zs_sft.jsonl (multimodal: image→IsingViT→LLM)"
        qc["init_max_points_per_centroid"] = None
        with open(qc_path, "w") as f:
            json.dump(qc, f, indent=2)
        qm.logger.info("[保存] quant_config.json 已更新 (2x16 scheme, 后8层, MLP-only)")
...
if __name__ == "__main__":
    ...
    qm.logger.info("=" * 60)
    qm.logger.info(" AQLM 2×16 量化 — 后8层 MLP-only (多模态校准)")
    qm.logger.info(f" scheme: 2×16 (2 codebooks, nbits=16, in_group=16)")
    qm.logger.info(f" 量化范围: 最后 8 层 (36层中的层28~35), 仅 MLP (gate/up/down)")
    qm.logger.info(f" Attention 层 (q/k/v/o) 保持原始精度，不量化")
    qm.logger.info(f" MLP:  codebook_size=65536 (nbits=16), ~3.4 bit/param")
    qm.logger.info(f" init_max_points_per_centroid=None (全量校准数据)")
