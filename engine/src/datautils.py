...
LOCAL_WIKITEXT2_DIR = Path(os.environ.get("LOCAL_WIKITEXT2_DIR", PROJECT_ROOT / "data" / "wikitext2"))
...
def _load_wikitext2_split(split: str):
    """Load wikitext-2-raw-v1 split from local parquet first, then HuggingFace Hub."""
    local_parquet = LOCAL_WIKITEXT2_DIR / f"{split}.parquet"
    if local_parquet.is_file():
        print(f"Loading wikitext2/{split} from local file: {local_parquet}")
        return load_dataset("parquet", data_files={split: str(local_parquet)}, split=split)

    try:
        return load_dataset("wikitext", "wikitext-2-raw-v1", split=split)
    except (ConnectionError, OSError) as err:
        raise ConnectionError(
            f"Failed to download wikitext2/{split} from HuggingFace Hub ({err}). "
            f"Place {split}.parquet under {LOCAL_WIKITEXT2_DIR} or run: "
            f"bash scripts/download_wikitext2.sh"
        ) from err
