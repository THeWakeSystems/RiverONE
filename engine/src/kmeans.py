...
def fit_faiss_kmeans(
    ...
    *,
    ...
    :param nredo: number of times to redo k-means (keep best run). FAISS supports nredo≥1.
    ...
    if max_points_per_centroid is not None:
        kmeans = faiss.Kmeans(
            d, k, niter=max_iter, nredo=nredo, verbose=verbose, gpu=gpu, max_points_per_centroid=max_points_per_centroid
        )
    else:
        kmeans = faiss.Kmeans(d, k, niter=max_iter, nredo=nredo, verbose=verbose, gpu=gpu)
...
@maybe_script
def find_nearest_cluster(data, clusters, block_size_vals: int = 2**27, devices: Optional[List[torch.device]] = None):
    """Find nearest clusters for each batch of data and return their indices
    block_size_vals 从 2**30 降至 2**27，限制单次 addmm 矩阵大小约 512MB，
    避免 OOM（尤其对于 1×16 方案中 codebook_size=65536 的场景）。"""
    if devices is None:
        devices = [data.device]
