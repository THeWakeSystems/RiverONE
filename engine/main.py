...
def get_inps(
    ...
    if emb_device.type != "cuda":
        ...
        if get_llm_config(model).model_type == "opt":
            forward_model.model.decoder.embed_positions = forward_model.model.decoder.embed_positions.to(device)
            if hasattr(forward_model.model.decoder, "project_in") and forward_model.model.decoder.project_in:
                forward_model.model.decoder.project_in = forward_model.model.decoder.project_in.to(device)
    ...
    if get_llm_config(model).model_type.lower() in FALCON_TYPES:
    ...
    for batch_inps in data:
        try:
            ...
            # call forward to trigger the Catcher (text-only path for InternVL)
            forward_model(batch_inps, attention_mask=torch.ones_like(batch_inps))
        except CatcherExit:
    ...
    if get_llm_config(model).model_type == "opt":
        forward_model.model.decoder.embed_positions = forward_model.model.decoder.embed_positions.to(emb_device)
        if hasattr(forward_model.model.decoder, "project_in") and forward_model.model.decoder.project_in:
            forward_model.model.decoder.project_in = forward_model.model.decoder.project_in.to(emb_device)
    ...
    for layer_index in range(len(layers)):
        ...
        for names in sequential:
            ...
            if len(args.devices) == 1:
                ...
                aq_handlers = init_aq_engines(
                    ...
                    [
                        name
                        for name in names
                        if ((".gate" not in name.lower()) or ("mixtral" not in get_llm_config(model).model_type.lower()))
                    ],
                    inps[0],
                    outs[0],
                    **forward_args,
            ...
            for sublayer_name in aq_handlers.keys():
                ...
                if "mixtral" in get_llm_config(model).model_type.lower() and args.mix_compression:
                    assert "mixtral" in get_llm_config(model).model_type.lower()
                ...
                quantizers["%s.%d.%s" % (get_quantizer_key_prefix(model), layer_index, sublayer_name)] = ()  # to be updated
...
def main():
    ...
    if args.devices is None:
        ...
        for device_str in args.devices:
            ...
            parsed_devices.append(torch.device(device_str))
        args.devices = parsed_devices
